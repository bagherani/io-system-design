...
Read the instructor notes from ./instructor-notes/instructor-notes.md
...
- Prioritize pedagogical readability over software engineering best practices: prefer concrete, hardcoded values and explicit step-by-step logic over loops, mapping, helper abstractions, or generic utility functions.
...
- Avoid port conflicts between lessons. Check existing examples before choosing new host ports. Current lesson-owned host ports include `3001`, `3013`, `3014`, `3015`, `3016`, `3017`, `3018`, `3019`, `3020`, `3021`, `3022`, `4101`, `4102`, `8088`, and `8443`; `3017` is owned by `01-redis`; `3018` is owned by `04-eda`; `3019` is owned by `01-rate-limiting`; `3020` (control panel API) and `3021` (nginx entry) are owned by `app` (Module 1 dashboard); `3022` is the Nginx entry point for `05-load-balancing`; MongoDB ports include `27031` for `03-be-db-app`, `27041`-`27043` for `04-replicaset`, `27050`-`27053` for `05-sharding`, and `27061` for `app`; Redis ports include `6371`-`6373` for `01-redis`; and `01-grpc` also reserves service port `5001` for local gRPC traffic.
